function [ TP,ID,state_est,P_est,width1,height1,bbox ] = Tracking(TP,PV,ID,OverlArea_thresh,ratioBB_thresh,...
...
if height(TP)>0 
    ...
    %%%%%%%%% a) UPDATE
            ...
            if TP.Flag(i)==0  %if this vehicle of TP has not been found in the new frame
                ...
                x_est=state_est(1,i);
                y_est=state_est(2,i);
                left_est=state_est(1,i)-(width1(i))/2;
                top_est=state_est(2,i)-(height1(i))/2;
                width_est=width1(i);
...
%                 TP.Missing(end)=0;  %counter missing =0
                state_est(:,end+1)=zeros(4,1);
                P_est(:,:,end+1)=zeros(4,4);
    ...
    %%%%%%%%% c) DISAPPEAR
       ...
       while i<=(height(TP)-1)
            ...
            while j<=height(TP)
                ...
                Aij = Hij*Wij; %common area between boundingboxes of two different frames
                Overlap_ij=Aij/min(TP.Area(j),TP.Area(i)); % overlapping area (%)
...
% 
%                 if Overlap_ij>0.8 || ... %2)
%                    ((TP.x(i)>TP.Left(j) && TP.x(i)<TP.Right(j))&&(TP.y(i)>TP.Top(j) && TP.y(i)<TP.Bottom(j)))...
%                    || ((TP.x(j)>TP.Left(i) && TP.x(j)<TP.Right(i))&&(TP.y(j)>TP.Top(i) && TP.y(j)<TP.Bottom(i)))
%                   
%                     if TP.Found(i)>TP.Found(j)
%                         del=[del,j];
%                     elseif TP.Found(i)<TP.Found(j)
%                         del=[del,i];  
%                     end
%               
%                 end
...
if height(TP)>0
    [bbox,state_est,P_est,width1,height1] = kalman1(TP,state_est,P_est,width1,height1);
else bbox=[];
end
